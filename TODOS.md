...
- [ ] **T31 (V2 — major direction)** — `/next-build` or `/level-up`: a longer, guided "major experience into Claude Code" that takes the PM beyond the 30-min ship moment into a real working relationship with the tool. /first-build's job is *transmit the feeling* in 30 minutes; T31's job is *teach the craft* in 2–4 hours, tiered by user level. **Hard constraint: strictly vanilla Claude Code surface — no gstack or other third-party skill dependencies** (builder clarification 2026-05-19). The PM finishing T31 should be self-sufficient with just Claude Code + git + GitHub. **Examples of what it would cover:** building something more functional and robust than first-build's static templates; authoring a proper project `CLAUDE.md`; adding tests and asking Claude to write them; QA workflows the PM runs themselves (no `/qa` skill dependency); customizing Claude Code via `settings.json`; writing your own skills (since they just used one); how to talk to Claude Code at the prompt level (file references, scope, intent); iterating safely with commits/branches; what shipping looks like in vanilla Claude Code (`git push` + PR via `gh` CLI or web UI — no third-party shipping skill). **Tiered by user level of need** — a non-technical PM picking it up after `/first-build` doesn't need the same depth as a technical PM who's done a dozen builds. Possibly supersedes T18 (laddered `/next-build`) — this is closer to a "major guided onboarding into Claude Code and all it can do" than a less-hand-holdy sister to first-build. **Builder's own workflow tools are fine for designing it** (the builder can use whatever skills they like internally); the *output* must stay vanilla. Surfaced by the builder mid-V1.5; vanilla-only constraint added 2026-05-19.
